...
const AnimatedDigit = ({ digit, delay = 0 }) => {
...
const NumberCounter = React.forwardRef(
  (
    {
      value,
      from = 0,
...
const RollingCounter = React.forwardRef(
        ...
        <span ref={ref} className="inline-flex">
          {isInView &&
            digits.map((digit, index) => (
              <AnimatedDigit key={`${index}-${digit}`} digit={digit} delay={index * 0.05} />
            ))}
        </span>
...
const CircularCounter = React.forwardRef(
  (
    {
      value,
      size = 120,
    ...
    return (
      ...
      >
        ...
        <div ref={ref} className="absolute inset-0 flex items-center justify-center">
          <span className="font-bold tabular-nums leading-none" style={{ fontSize: size * 0.22 }}>
