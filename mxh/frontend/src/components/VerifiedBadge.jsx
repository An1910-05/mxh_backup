...
import VerifiedPurchaseModal from './VerifiedPurchaseModal';

export default function VerifiedBadge({ isVerified, verifiedUntil, ownerId, onPurchased, size = 16 }) {
  const { user } = useAuth();
  ...
  const handleClick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (isOwn || isVerified) setShowModal(true);
  };
  ...
  return (
    <>
      <button
        type="button"
        className={`verified-badge${isVerified ? ' verified-badge--active' : ' verified-badge--gray'}`}
        onClick={handleClick}
        title={isVerified ? 'Đã xác thực' : 'Mua tích xanh'}
        style={{ width: size, height: size }}
        aria-label={isVerified ? 'Đã xác thực' : 'Mua tích xanh'}
      >
        <svg viewBox="92 0 24 24" width={size} height={size} fill="currentColor" aria-hidden>
      ...
      {showModal && (
        <VerifiedPurchaseModal
          ...
          onPurchased={(profile) => {
            setShowModal(false);
            if (onPurchased) onPurchased(profile);
          }}
        />
      )}
    </>
