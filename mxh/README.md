...
- **Trang cá nhân — Chế độ riêng tư (Public / Private):** Người dùng có thể đặt trang cá nhân ở chế độ **Công khai** (mặc định) hoặc **Riêng tư**. Riêng tư: chỉ **bạn bè** (`friendships.status='accepted'`) mới xem được bài viết, story, danh sách bạn/follower và thông tin trang cá nhân; người theo dõi không phải bạn bè cũng bị chặn. Người lạ thấy màn hình khóa "Tài khoản này ở chế độ riêng tư" + nút kết bạn (tài khoản vẫn tìm được trong tìm kiếm và vẫn nhận lời mời kết bạn). Cổng kiểm tra tập trung `PrivacyService::canView(viewer, owner)`; feed nhiều tác giả (`posts`/`feed`/`feedStories`) lọc ngay ở SQL nên phân trang vẫn đúng. Bật/tắt ở **Cài đặt → Quyền riêng tư**.
  - Migration: `backend/database/migrations/031_add_profile_privacy.sql` (thêm cột `users.is_private`)
  - Backend: `src/Services/PrivacyService.php` (mới), `ProfileService.php` (is_private/is_locked + `setPrivacy`), `PostService.php`, `StoryService.php`, `Repositories/UserRepository.php` (`setPrivacy`, is_private trong findById), `PostRepository.php`, `StoryRepository.php`, `GraphQL/Types/ProfileType.php` (field `is_private`,`is_locked`), `GraphQL/Mutations/MutationType.php` (mutation `setPrivacy`), `GraphQL/Queries/QueryType.php` (gate `userStories`/`userFriends`/`userFollowers`/`userFollowing`)
  - Frontend: `services/graphql.js` (`setPrivacy` + field mới), `pages/SettingsPage.jsx` (mục "Quyền riêng tư"), `pages/ProfilePage.jsx` + `components/ProfileInfo.jsx` (màn hình khóa, prop `locked`), `styles.css`
  - Lưu ý: thay đổi backend cần **Rebuild (bước 2 trong start.cmd)** để có hiệu lực; migration 031 tự chạy qua `do_migrate`.
