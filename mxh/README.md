...
- **Settings — Redesign giao diện với Bootstrap Icons + hiệu ứng JolyUI:** Trang `/settings` được tái thiết kế hoàn toàn theo phong cách hiện đại (glassmorphism, ambient blob background, animated gradient headline) với 4 component JolyUI mới (`BorderBeam` cho item sidebar đang chọn + viền wallet card, `ShimmerButton` cho nút Lưu/Đổi mật khẩu, `MagicCard` spotlight cho feature chip, `Meteors` shower phủ wallet card). Toàn bộ icon SF Symbols PNG được thay bằng **Bootstrap Icons** (`bi-*` font, nạp qua CDN `cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3`). Sidebar có icon box gradient + hint phụ + smooth nav transition; mỗi section dùng `motion.div` + `AnimatePresence` để fade-in khi đổi tab.
